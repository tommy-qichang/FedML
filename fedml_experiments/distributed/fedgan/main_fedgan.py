...
from fedml_api.data_preprocessing.brats.data_loader_gan import load_partition_data_distributed_brats
...
from fedml_api.distributed.fedgan.utils import count_parameters
...
def create_model(args, model_name, device):
    if model_name.lower() == 'dadgan':
        model = DadganModel(args, device)

        num_params = count_parameters(model)
        logging.info("dadgan Model Size = {0} M".format(str(num_params)))
    else:
...
if __name__ == "__main__":
    ...
    logging.info("Executing FedGAN at time: {0}".format(time_start))
    ...
    if process_id == 0:
        wandb.init(
            project="fedml",
            name="FedGan(d)" + args.dataset + '-' + str(args.partition_method) + "-r" + str(args.comm_round) + "-e" + str(
                args.epochs) + "-lr" + str(
                args.lr),
            config=args
        )
    ...
    # create model.
    model = create_model(args, model_name=args.model, device=device)

    logging.info("Calling FedML_FedGan_distributed")

    # start "federated GAN (FedGan)"
    FedML_FedGan_distributed(process_id, worker_number, device, comm, model, train_data_num, train_data_global, test_data_global, data_local_num_dict,
