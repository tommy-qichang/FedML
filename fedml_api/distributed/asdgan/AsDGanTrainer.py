...
class AsDGanTrainer(object):
    def __init__(self, client_index, train_data_local_dict, train_data_local_num_dict,
                 train_data_num, test_data_local_dict, device, args, model_trainer):
    ...
    def upload_labels(self):
        ...
        return self.local_sample_number, keys, labels

    def train(self, key_samples, fake_samples, transform_params):
        data_batch, label_batch = self.train_dataset_local.get_data(key_samples, transform_params)

        train_metrics, grad_fake_B = self.trainer.train_one_iter(label_batch, data_batch, fake_samples)

        return grad_fake_B, self.local_sample_number, train_metrics
