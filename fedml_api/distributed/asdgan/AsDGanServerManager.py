...
from fedml_api.distributed.asdgan.utils import transform_tensor_to_list
...
class AsDGanServerManager(ServerManager):
    ...
    def handle_message_receive_grad_from_client(self, msg_params):
        ...
        self.aggregator.add_local_grad(sender_id - 1, grad_fake_sample, local_sample_number, model_params)
    ...
    def send_message_fake_sample_to_client(self, receive_id, data, client_index, is_next_epoch):
        ...
        message.add_params(MyMessage.MSG_ARG_KEY_FAKE_SAMPLES_AUG_PARAMS, data['tsfm'])

        message.add_params(MyMessage.MSG_ARG_KEY_CLIENT_INDEX, str(client_index))
        message.add_params(MyMessage.MSG_ARG_KEY_EPOCH_INDEX, is_next_epoch)
        # logging.info(f"[Server] send fake samples with key:{data['key']}")
        self.send_message(message)
