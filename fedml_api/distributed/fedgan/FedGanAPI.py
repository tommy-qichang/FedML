...
def init_server(args, device, comm, rank, size, model, model_trainer, train_data_global, test_data_global):
    ...
    # aggregator
    worker_num = size - 1
    aggregator = FedGanAggregator(worker_num, device, model, args, model_trainer, train_data_global, test_data_global)

    # start the distributed training
    server_manager = FedGanServerManager(args, aggregator, comm, rank, size)
...
def init_client(args, device, comm, process_id, size, model, train_data_num, train_data_local_num_dict,
    ...
    # trainer
    trainer = FedGanTrainer(client_index, train_data_local_dict, train_data_local_num_dict, train_data_num, test_data_local_dict, device,
                            args, model_trainer)
    client_manager = FedGanClientManager(args, trainer, comm, process_id, size)
    client_manager.run()
